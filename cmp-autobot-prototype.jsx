...
import { LayoutDashboard, ListChecks, ClipboardList, History, Search, X, Copy, ExternalLink, Check, ChevronDown, ChevronRight, Plus, AlertTriangle, Wifi, Clock, ArrowRight, RotateCcw, Filter } from 'lucide-react';
...
const BLUE_REPLACEMENTS = {
  'mog-013': [
    { id: 'apl-b1', name: 'Nature\'s Bliss', pack: '1L', cost: 180, topPick: true, chips: [{ label: 'Cold pressed', type: 'positive' }, { label: 'Pack match', type: 'positive' }] },
    { id: 'apl-b2', name: 'Aroma Roots', pack: '1L', cost: 220, chips: [{ label: 'Cold pressed', type: 'positive' }, { label: 'Higher cost', type: 'attention' }] },
    { id: 'apl-b3', name: 'Pure Leaf Essentials', pack: '500ml', cost: 175, chips: [{ label: 'Smaller pack', type: 'attention' }] },
  ],
  'mog-014': [
    { id: 'apl-b4', name: 'Ashirwad Select', pack: '10kg', cost: 52, topPick: true, chips: [{ label: 'Closest brand', type: 'positive' }, { label: 'Pack match', type: 'positive' }] },
    { id: 'apl-b5', name: 'Pillsbury Chakki Fresh', pack: '10kg', cost: 58, chips: [{ label: 'Pack match', type: 'positive' }, { label: 'Different brand', type: 'attention' }] },
  ],
  'mog-015': [],
};
...
const QueueTag = ({ queue, children, size = 'sm' }) => {
  const styles = {
    green: { bg: '#EAF3DE', text: '#173404', border: '#639922' },
    amber: { bg: '#FAEEDA', text: '#412402', border: '#EF9F27' },
    red: { bg: '#FCEBEB', text: '#501313', border: '#E24B4A' },
    blue: { bg: '#E6F1FB', text: '#042C53', border: '#378ADD' },
    neutral: { bg: '#F1EFE8', text: '#2C2C2A', border: '#B4B2A9' },
  };
...
const Button = ({ variant = 'secondary', size = 'md', children, onClick, disabled }) => {
  ...
  const sizes = {
    ...
    md: { fontSize: '12px', padding: '7px 12px' },
    lg: { fontSize: '13px', padding: '9px 16px' },
  };
...
const NavSidebar = ({ active, onNavigate, myTasksCount }) => {
  ...
  return (
    <div style={{ background: '#FFFFFF', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: '8px', padding: '14px 10px', height: 'fit-content', position: 'sticky', top: '16px' }}>
...
const QueueList = ({ mogs, activeTab, onTabChange, selectedMogId, onSelectMog, taskDraftState }) => {
  ...
  const tabs = [
    { id: 'amber', label: 'Amber', sub: 'Needs review' },
    { id: 'green', label: 'Green', sub: 'Matched' },
    { id: 'red', label: 'Red', sub: 'No match' },
    { id: 'blue', label: 'Blue', sub: 'Transition' },
  ];
    ...
    <div style={{ background: '#FFFFFF', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: '8px', padding: '12px 10px', height: 'fit-content' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', maxHeight: '600px', overflowY: 'auto' }}>
        {filtered.map(mog => {
          ...
          return (
            <div key={mog.id} onClick={() => onSelectMog(mog.id)} style={{ padding: '9px 10px', borderLeft: `2px solid ${isSelected ? tabColor : 'transparent'}`, background: isSelected ? tabBg : 'transparent', borderRadius: '0 4px 4px 0', marginBottom: '2px', cursor: 'pointer', transition: 'all 0.1s' }}>
              ...
              {mog.queue === 'amber' && <p style={{ fontSize: '10px', color: '#888780', margin: 0 }}>{(AMBER_CANDIDATES[mog.id] || []).length} candidates{draftSelections ? ` · ${draftSelections.length} selected` : ''}</p>}
...
const RedDetail = ({ mog, onEscalate, onManualAdd }) => {
...
const HandoffModal = ({ task, onClose, onComplete }) => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px' }}>
      <div style={{ background: '#FFFFFF', borderRadius: '12px', maxWidth: '540px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '0 20px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
            <Button variant="secondary" size="md" onClick={() => navigator.clipboard?.writeText(packageText)}><Copy size={12} /> Copy details</Button>
...
const MyTasks = ({ tasks, onOpenTask, onRemove }) => {
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: '12px', color: '#5F5E5A', margin: '0 0 4px' }}>
                    {task.type === 'escalation' ? 'No credible APL — escalated to Procurement Team · Awaiting resolution' :
                     ...
                     task.action === 'replace' ? <>
                       <span style={{ textDecoration: 'line-through', color: '#888780' }}>{task.retiredAPL}</span> → Link <span style={{ color: '#2C2C2A', fontWeight: 500 }}>{task.apls[0].name}</span> · Atomic swap: delink + link together
                     </> : <>
...
const Toast = ({ message, onClose }) => {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '24px', left: '50%', transform: 'translateX(-50%)', background: '#2C2C2A', color: '#FFFFFF', padding: '10px 16px', borderRadius: '8px', fontSize: '13px', zIndex: 200, display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
...
export default function App() {
  ...
  const handleConfirmMapping = (apls, defaultAPL) => {
    ...
    setMogs(mogs.filter(m => m.id !== selectedMog.id));
    const nextMog = filteredMogs.find(m => m.id !== selectedMog.id);
    if (nextMog) setSelectedMogId(nextMog.id);
    setToast(`${selectedMog.name} confirmed → My Tasks`);
    const newDrafts = { ...draftSelections };
    delete newDrafts[selectedMog.id];
  ...
  const handleTaskComplete = (continueNext) => {
